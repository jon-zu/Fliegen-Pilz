...
public class SendOpcodesExt
{
    public static SendOpcodes GetSendOpcode(short opcode)
    {
        if (Enum.IsDefined(typeof(SendOpcodes), opcode))
        {
            return (SendOpcodes)opcode;
        }
        else
        {
            throw new ArgumentOutOfRangeException(nameof(opcode), $"Invalid SendOpcode: {opcode}");
        }
    }
}